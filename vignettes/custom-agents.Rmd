...
vignette: >
  %\VignetteIndexEntry{Building Custom Agents}
...
If you already use built-in functions like `build_code_agent()` and
`build_researcher_agent()`, custom agents let you define your own workflow
steps.
...
- `node`: a function `function(state)` that returns a named list update, or
  `make_command(goto = "...", update = list(...))`.
- `edge`: a transition from one node to the next.
- `entry_point`: the first node to run.
- `__end__`: special terminal node.

You build this with `build_custom_agent()` or `compile_graph()`.
...
Use `compile_graph()` when you want both:
...
- Node returns non-list output:
  return a named list or `make_command(...)`, not a bare string/number.
- `entry_point` typo:
  it must match one of the names in `node_functions`.
- Missing route mapping:
  if `condition(state)` returns a label not found in `mapping`, execution fails.
- No path to `__end__`:
  make sure every route eventually terminates.
...
See: `vignettes/multi-agent-teams.Rmd` for the full beginner walkthrough.
